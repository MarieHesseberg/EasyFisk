...
test("Vipps can be cancelled and completed without secure-context randomUUID", async ({
  ...
  await page.getByRole("button", { name: "Mine fiskekort", exact: true }).click();
  await expect(page.getByRole("heading", { name: "Mine fiskekort", exact: true })).toBeVisible();
  await page.getByRole("button", { name: "Hjem", exact: true }).click();
  await page.getByRole("button", { name: "Kjøp fiskekort", exact: true }).click();
  await expect(shop.locator("article").first()).toBeVisible();
  await expect(shop.getByRole("heading", { name: "Betaling godkjent" })).toHaveCount(0);
